...
#pragma endregion
	...
	private: System::Void listBox1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		String^ balls = context.marshal_as<String^>(datahash);
		...
		fileSize = 0;
		unsigned char* datdata = new unsigned char[datsize];
		pkg.readHeader();
		pkg.getEntryTable();
		Entry dataEntry = pkg.entries[(hexStrToUint32(datahash)%8192)];
		dataEntry.fileSize = datsize;
		datdata = pkg.getBufferFromEntry(dataEntry);
		MessageBox::Show("dathash: " + context.marshal_as<String^>(dataEntry.reference) + " filesize? " + dataEntry.fileSize.ToString());

		if (datdata == nullptr || sizeof(datdata) == 0) {
			MessageBox::Show("Data sized 0. Exiting");
			exit(120);
		}
		...
		fwrite(datdata, dataEntry.fileSize, 1, dFile);
		fclose(dFile);
		delete[] datdata;
