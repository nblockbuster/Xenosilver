...
#pragma endregion
	...
	private: System::Void listBox1_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		std::string dathash = getReferenceFromHash(hash, pkgsPath);

		pkgID = getPkgID(dathash);

		Package pkg(pkgID, pkgsPath);
		String^ hash2 = context.marshal_as<String^>(dathash);
		...
		data = pkg.getEntryData(dathash, fileSize);
		...
		int index = hexStrToUint32(hash) % 8192;
		MessageBox::Show(index.ToString());
		int filesizeentry = pkg.entries[index].fileSize;
		MessageBox::Show(filesizeentry.ToString());
		FILE* dFile;
		fopen_s(&dFile, "expath_temp/temp.dat", "w");
		fwrite(data, filesizeentry, 1, dFile);
		fclose(dFile);
		delete[] data;
